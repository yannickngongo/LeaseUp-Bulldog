...
<head>
<meta charset="utf-8"/>
<title>LeaseUp Bulldog — ${monthLabel} Report — ${operatorName}</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  h2 { font-size: 15px; font-weight: 700; margin: 24px 0 12px; border-bottom: 1px solid #eee; padding-bottom: 6px; }
  .sub { font-size: 12px; color: #666; margin-bottom: 24px; }
  .badge { display: inline-block; padding: 3px 10px; border-radius: 100px; font-size: 11px; font-weight: 700; background: #ecfdf5; color: #065F46; margin-bottom: 12px; }
  .badge.warn { background: #fffbeb; color: #b45309; }
  .badge.crit { background: #fef2f2; color: #991b1b; }
  .kpi-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 24px; }
  .kpi { border: 1px solid #e5e7eb; border-radius: 12px; padding: 14px; }
  .kpi-val { font-size: 22px; font-weight: 900; }
  .kpi-lbl { font-size: 10px; color: #666; margin-top: 2px; }
  .summary { border: 1px solid #e5e7eb; border-radius: 12px; padding: 16px; margin-bottom: 16px; font-size: 13px; line-height: 1.6; }
  .note { border: 1px solid #fca5a5; border-radius: 12px; padding: 14px; margin-bottom: 16px; background: #fef2f2; }
  .note-label { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: #C8102E; margin-bottom: 4px; }
  .note-text { font-size: 13px; font-weight: 600; color: #111; }
  ...
  .section-title { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: #888; margin-bottom: 8px; }
  ul { list-style: none; }
  li { font-size: 12px; color: #374151; padding: 3px 0; display: flex; gap: 6px; }
  ol { list-style: none; counter-reset: item; }
  ol li { counter-increment: item; }
  ol li::before { content: counter(item); display: inline-flex; width: 18px; height: 18px; border-radius: 50%; background: #fee2e2; color: #C8102E; font-size: 9px; font-weight: 900; align-items: center; justify-content: center; flex-shrink: 0; margin-top: 1px; }
  .footer { margin-top: 32px; font-size: 10px; color: #999; border-top: 1px solid #eee; padding-top: 12px; }
  @media print { body { padding: 20px; } @page { margin: 1cm; } }
</style>
</head>
<body>
<h1>LeaseUp Bulldog — ${monthLabel}</h1>
<div class="sub">Prepared for ${operatorName} · Generated ${new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</div>

<div class="badge ${aiReport.performance_rating === 'Excellent' ? '' : aiReport.performance_rating === 'Strong' ? '' : aiReport.performance_rating === 'Needs Attention' ? 'warn' : 'crit'}">${aiReport.performance_rating}</div>

<div class="kpi-grid">
  <div class="kpi"><div class="kpi-val">${data.occupancy_pct}%</div><div class="kpi-lbl">Portfolio Occupancy</div></div>
  <div class="kpi"><div class="kpi-val">${data.leads_qualified}</div><div class="kpi-lbl">Leads Qualified</div></div>
  <div class="kpi"><div class="kpi-val">${data.leases_attributed}</div><div class="kpi-lbl">Leases Attributed</div></div>
  <div class="kpi"><div class="kpi-val">$${data.revenue_impact.toLocaleString()}</div><div class="kpi-lbl">Revenue Impact</div></div>
</div>

<h2>Executive Summary</h2>
<div class="summary">${aiReport.executive_summary}</div>

<div class="note">
  <div class="note-label">Key Takeaway for Owner</div>
...
<div class="two-col">
  <div class="section">
    ...
    <ul>${aiReport.highlights.map(h => `<li><span style="color:#10b981">✓</span>${h}</li>`).join("")}</ul>
  </div>
  <div class="section">
    <div class="section-title">Risks to Watch</div>
    <ul>${aiReport.risks.map(r => `<li><span style="color:#ef4444">⚠</span>${r}</li>`).join("")}</ul>
  </div>
</div>
...
<div class="footer">Generated by LeaseUp Bulldog · ${data.property_count} properties · ${data.total_units} total units · ${data.ai_messages_sent.toLocaleString()} AI messages · ${data.avg_response_time_min} min avg response</div>
</body>
</html>`;
                ...
                <div className="flex flex-col gap-3 rounded-xl border border-gray-100 dark:border-white/5 bg-white dark:bg-[#1C1F2E] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex gap-2">
                    <button onClick={exportPdf}
                      className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 rounded-lg bg-[#C8102E] px-4 py-2 text-xs font-bold text-white hover:bg-[#A50D25]">
